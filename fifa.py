...
class FIFA(commands.Cog):
    ...
    async def pack(self, interaction: discord.Interaction):
        ...
        image_path = os.path.join(IMAGES_PATH, player.get("image", ""))
        if player.get("image") and os.path.exists(image_path):
            file = discord.File(image_path, filename=player["image"])
